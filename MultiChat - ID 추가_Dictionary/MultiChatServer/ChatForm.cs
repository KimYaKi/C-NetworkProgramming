        ...
        void DataReceived(IAsyncResult ar) {
            ...
            if (received <= 0) {
                ...
                if (clientNum > 0)
                {
                    ...
                    {
                        // 접속을 끊은 client의 socket값을 검사해서 삭제한다.
            ...
            if (tokens[0].Equals("id"))
            {
                ...
                // obj.WorkingSocket : id의 값을 보낸 client의 socket으로 저장한다.
            }
            else
            {
                switch (code_tokens[0])
                {
                    case "200":
                        id = tokens[0];
                        string msg = tokens[1];
