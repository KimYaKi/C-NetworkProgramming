...
namespace MultiChatServer {
    public partial class ChatForm : Form {
        ...
        List<AsyncObject> connectedClients;
        ...
        public ChatForm() {
            ...
            connectedClients = new List<AsyncObject>();
            clientNum = 0; //초기화
        }
        ...
        void DataReceived(IAsyncResult ar) {
            ...
            if (received <= 0) {
               // AppendText(txtHistory, string.Format("클라이언트 접속해제?{0}", clientNum));
                if(clientNum > 0)
                {
                    foreach (AsyncObject o in connectedClients)
                    {
                        if (obj.WorkingSocket == o.WorkingSocket)
                        {
                            try
                            {
                                connectedClients.Remove(o);
                                AppendText(txtHistory, string.Format("접속해제완료:{0}", obj.id));
                            }
                            catch { }
                            break;
                        }
            ...
            //AppendText(txtHistory, tokens.ToString());
            if (tokens[0].Equals("ID"))    // 받은 문자열   id:자신의 id
            {
                clientNum++;
                obj.id = tokens[1];
                AppendText(txtHistory, string.Format("[접속{0}]ID:{1}:{2}", 
                       clientNum, obj.id, obj.WorkingSocket.RemoteEndPoint.ToString()));
            ...
            {
                string fromID = tokens[0].Trim(); //fromID
                ...
                {
                    string toID = tokens[2].Trim();
                    string msg = tokens[3];
                    string sendingMsg = "[FROM:" + fromID + "][TO:" + toID+ "]"+ msg;
                    AppendText(txtHistory, sendingMsg+"["+toID+"}");
        ...
        void sendTo(string id, byte[] buffer)
        {

            foreach (AsyncObject obj in connectedClients)
            {
                //char[] oid = obj.id.ToCharArray();
                //char[] cid = id.ToCharArray();
                //bool result = false; 
                //AppendText(txtHistory, oid.Length.ToString() + ":" + cid.Length.ToString());
                //for (int i=0; i<oid.Length; i++)
                //{
                //    result = oid[i] == cid[i];
                //    AppendText(txtHistory, i + ":" + result);

                //}
                   
                //AppendText(txtHistory, string.Equals(obj.id, id).ToString() + "=" + id + "==" + obj.id + "=");
                //AppendText(txtHistory, "To socket" + obj.WorkingSocket.RemoteEndPoint.ToString());
               
                if (string.Equals(obj.id, id))
                {
                    try { obj.WorkingSocket.Send(buffer); }
                    catch
                    {// 오류 발생하면 전송 취소하고 삭제
                        try
                        {
                            AppendText(txtHistory, id + "=dispose");
        ...
        void sendAll(Socket except, byte[] buffer)
        {
            foreach (AsyncObject obj in connectedClients)
            {
                if (obj.WorkingSocket != except)
                {
                    try { obj.WorkingSocket.Send(buffer); }
                    catch
                    {// 오류 발생하면 전송 취소하고 삭제
                        try { obj.WorkingSocket.Dispose(); } catch { }
                    }
                }
            }
        }
